function varargout=suit_isolate_gio(Source,varargin);
...
if (strcmp(crop,'crop'))
    ...
    % Crop the volumes around the cerebellum for diffusion and SMT images
%     diff=('data.nii');                                                                              % aggiunto da Gio
%     [diff_dir,diff,diff_ext,diff_num]=spm_fileparts(diff);                                          % aggiunto da Gio
%     diffc=fullfile(diff_dir,diff);                                                                  % aggiunto da Gio
%     crop_diff=fullfile(diff_dir,['c_' diff diff_ext]);                                              % aggiunto da Gio
%     graddev=('grad_dev.nii');                                                                       % aggiunto da Gio
%     [graddev_dir,graddev,graddev_ext,graddev_num]=spm_fileparts(graddev);                           % aggiunto da Gio
%     graddevc=fullfile(graddev_dir,graddev);                                                         % aggiunto da Gio
%     crop_graddev=fullfile(graddev_dir,['c_' graddev graddev_ext]);                                  % aggiunto da Gio
%     SMT=('SMT_mcmicro_output.nii');                                                                 % aggiunto da Gio
%     [SMT_dir,SMT,SMT_ext,SMT_num]=spm_fileparts(SMT);                                               % aggiunto da Gio
%     SMTc=fullfile(SMT_dir,SMT);                                                                     % aggiunto da Gio
%     crop_SMT=fullfile(SMT_dir,['c_' SMT SMT_ext]);                                                  % aggiunto da Gio
%     dti=('dti.nii');                                                                                % aggiunto da Gio
%     [dti_dir,dti,dti_ext,dti_num]=spm_fileparts(dti);                                               % aggiunto da Gio
%     dtic=fullfile(dti_dir,dti);                                                                     % aggiunto da Gio
%     crop_dti=fullfile(dti_dir,['c_' dti dti_ext]);                                                  % aggiunto da Gio
    t1t2=('T1w-T2w_125.nii');                                                                       % aggiunto da Gio
    [t1t2_dir,t1t2,t1t2_ext,t1t2_num]=spm_fileparts(t1t2);                                          % aggiunto da Gio
    t1t2c=fullfile(t1t2_dir,t1t2);                                                                  % aggiunto da Gio
    crop_t1t2=fullfile(t1t2_dir,['c_' t1t2 t1t2_ext]);                                              % aggiunto da Gio
    noise=('b0_NOISE.nii');                                                                         % aggiunto da Gio
    [noise_dir,noise,noise_ext,noise_num]=spm_fileparts(noise);                                     % aggiunto da Gio
    noisec=fullfile(noise_dir,noise);                                                               % aggiunto da Gio
    crop_noise=fullfile(noise_dir,['c_' noise noise_ext]);                                          % aggiunto da Gio
    try
...
%         spmj_crop_vol([diffc diff_ext],crop_diff,cropped_bb.x,cropped_bb.y,cropped_bb.z);           % aggiunto da Gio
%         spmj_crop_vol([graddevc graddev_ext],crop_graddev,cropped_bb.x,cropped_bb.y,cropped_bb.z);  % aggiunto da Gio
%         spmj_crop_vol([SMTc SMT_ext],crop_SMT,cropped_bb.x,cropped_bb.y,cropped_bb.z);              % aggiunto da Gio
%         spmj_crop_vol([dtic dti_ext],crop_dti,cropped_bb.x,cropped_bb.y,cropped_bb.z);              % aggiunto da Gio
        spmj_crop_vol([t1t2c t1t2_ext],crop_t1t2,cropped_bb.x,cropped_bb.y,cropped_bb.z);           % aggiunto da Gio
        spmj_crop_vol([noisec noise_ext],crop_noise,cropped_bb.x,cropped_bb.y,cropped_bb.z);        % aggiunto da Gio
...
if (iterations>1)
    ...
    fprintf('done\n');
    suit_isolate_gio([Sourcec ext],'Ms2a',Ms2a,'prior',[prior_dir '/' prior2],...
